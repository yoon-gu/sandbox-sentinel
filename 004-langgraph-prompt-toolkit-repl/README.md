...
```bash
cd 004-langgraph-prompt-toolkit-repl
...
| 상황 | 추천 |
|---|---|
| 사내 미러에 `textual` 이 공식 등록되어 있음 | **003** (textual) — 좀 더 리치한 스타일 |
| `textual` 이 없거나 확인 어려움 / 최소 의존 선호 | **004** (prompt_toolkit) — ipython 이 있으면 거의 확실히 동작 |
| 사내에 ipython 도 없다 (매우 드뭄) | 둘 다 안 됨 — 더 기초적인 stdlib-only REPL 을 새로 만들어야 함 |

기능적으로는 UX 차이가 거의 없으니 **스택 적합성** 기준으로 고르면 됩니다.
