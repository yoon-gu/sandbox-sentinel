...
├── basic_usage.py          # CLI/스크립트형 변환물 한정 (TUI · pure Python lib 등)
├── demo.ipynb              # 노트북 변환물 한정 (ipywidgets/HTML 등 Jupyter 안에서 동작)
└── data/                   # 샘플 데이터 (선택, 작은 파일만)
```

> 예제 파일은 변환물 폴더 **루트** 에 둡니다 — `examples/` 같은 하위
> 디렉토리는 만들지 않습니다 (계층을 얕게 유지).
...
- **사용 예시**: 예제 파일(`basic_usage.py` 또는 `demo.ipynb`) 참조 안내
- **알려진 제약/한계점**
...
- **`examples/` 같은 하위 디렉토리는 만들지 않음** — 폴더 구조를 얕게 유지
...
### 일관성 원칙
- 기본 구조는 모든 폴더에 적용. **필수**: `README.md`, `<main>.py`, `metadata.json`, `LICENSE`, 그리고 변환물 형태에 맞는 예제 (`basic_usage.py` 또는 `demo.ipynb`) 1개 이상.
...
- **예제 스크립트 (`basic_usage.py`)**: `print()` 사용 가능
