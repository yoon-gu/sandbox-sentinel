...
- **레이아웃**:
  ...
  - 우:
    ...
    - **💡 추천 라인** — 컨텍스트 라벨 + 상위 14개 추천 (색상별 kind 구분)
...
| 키 | 동작 |
|---|---|
| **Tab** | 자동완성 popup (수동 트리거 · 편집 중 언제든) |
...
식별자 글자(영문/숫자/`_`/`.`)를 칠 때마다 컨텍스트 인식 추천 popup 이 **자동으로** 노출됩니다 (CodeMirror 의 inline popup 과 같은 UX).

- `Enter` — 선택 후 커서 위치에 인서트
- `Esc` 또는 `Tab` — popup 닫기. **같은 단어를 계속 치는 동안엔 다시 안 뜸** (방해 안 함). 단어 경계(공백·`,`·`(` 등)를 넘으면 자동 재개.
